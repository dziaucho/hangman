import { nameInput, surnameInput } from "../elements/entry-page/entry-page";
...
export function createUser(): void {
  ...
  localStorage.setItem(`${user.name}_${user.surname}`, JSON.stringify({ user }));
...
function updateUsersLogged(newUserName: string, newUserSurname: string): void {
  ...
  for (let key of keys) {
    const storedUser = JSON.parse(localStorage.getItem(key)!);
    if (storedUser.name !== newUserName || storedUser.surname !== newUserSurname) {
